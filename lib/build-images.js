...
const singleFigureJobs = [
  {
    src: 'portrait.png',
    flip: false,
    // Straight 4:5 crop centered on the face; also trims the dark band along the
    // bottom edge of the source file. No retouching or filters (CLAUDE.md).
    extract: { left: 75, top: 0, width: 624, height: 780 },
    base: 'portrait',
    sizes: [
      { suffix: 'sm', width: 400, height: 500, quality: 82 },
      { suffix: '', width: 624, height: 780, quality: 86 },
    ],
  },
];
...
async function buildImages() {
  ...
  for (const job of singleFigureJobs) {
    ...
    const base = sharp(srcPath).rotate();
    let oriented = job.flip ? base.flop() : base;
    if (job.extract) oriented = oriented.extract(job.extract);
